...
- [x] #2 A ciphertext moved to a different column or a different rows cell fails authentication on read
...
The row half is now covered by TASK-0127 (feat/task-0127-row-binding): with a declared [[table]] row_key the envelope binds key_id || column || row key as DBS3, so cross-row relocation within one column fails authentication. Cross-column and cross-table were already covered by the DBS2 context binding.
<!-- SECTION:NOTES:END -->
